...
const checksum1 = crypto.createHmac('sha256', checksumSecret).update(canonicalPayload).digest('hex');
...
const checksum2 = crypto.createHmac('sha256', checksumSecret).update(values).digest('hex');
...
const checksum3 = crypto.createHmac('sha256', checksumSecret).update(payload3).digest('hex');
...
const checksum4 = crypto.createHmac('sha256', checksumSecret).update(base64Payload).digest('hex');
...
console.log('Try each checksum above in ClickPesa API and let me know which one works!');
