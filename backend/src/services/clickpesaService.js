...
class ClickPesaService {
  ...
  async generateToken() {
    try {
      const response = await axios.post(`${this.baseUrl}/generate-token`, {}, {
        headers: {
          'api-key': this.apiKey,
      ...
      console.error('ClickPesa Token Generation Error:', error.response?.data || error.message);
      throw new Error('Failed to generate ClickPesa token');
    }
  }
  ...
  async checkPaymentStatus(orderReference) {
    try {
      ...
      const response = await axios.get(`${this.baseUrl}/payments/query-all-payments?orderReference=${orderReference}`, {
        headers: {
          'Authorization': token,
          'Content-Type': 'application/json'
        }
      ...
      console.error('ClickPesa Status Check Error:', error.response?.data || error.message);
  ...
  async getBalance() {
    try {
      ...
      console.error('ClickPesa Balance Error:', error.response?.data || error.message);
  ...
  async previewCardPayment(amount, orderReference) {
    try {
      ...
      const data = {
        ...
        currency: 'USD',
        orderReference,
        checksum: this.generateChecksum({ amount, currency: 'USD', orderReference })
      };

      const response = await axios.post(`${this.baseUrl}/payments/preview-card-payment`, data, {
        headers: {
          'Authorization': token,
          'Content-Type': 'application/json'
        }
      ...
      console.error('ClickPesa Preview Card Payment Error:', error.response?.data || error.message);
  ...
  async initiateCardPayment(amount, orderReference, customerId) {
    try {
      ...
      const data = {
        ...
        checksum: this.generateChecksum({ amount, currency: 'USD', orderReference, customer: { id: customerId } })
      };

      const response = await axios.post(`${this.baseUrl}/payments/initiate-card-payment`, data, {
        headers: {
          'Authorization': token,
          'Content-Type': 'application/json'
        }
      ...
      console.error('ClickPesa Initiate Card Payment Error:', error.response?.data || error.message);
  ...
  async getExchangeRate() {
    try {
      ...
      const response = await axios.get('https://www.bot.go.tz/api/exchangerates');
