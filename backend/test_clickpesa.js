...
async function testClickPesaIntegration() {
  ...
  try {
    ...
    try {
      ...
      console.log('   This usually means your ClickPesa credentials are invalid');
      return;
    }
    ...
    try {
      ...
      const preview = await clickpesaService.previewPayment(testPhone, testAmount, testOrderRef);
    ...
    try {
      ...
      if (!testPhone.startsWith('255') || testPhone.length !== 12) {
        console.log('   ❌ Invalid phone format. Should be 255XXXXXXXXX (12 digits)');
        return;
      }
      ...
      const payment = await clickpesaService.initiatePayment(testPhone, testAmount, testOrderRef);
