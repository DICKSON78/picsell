...
async function testUSSDPayment() {
  ...
  try {
    ...
    try {
      const preview = await clickpesaService.previewPayment(testPhone, testAmount, orderRef);
    ...
    try {
      const payment = await clickpesaService.initiatePayment(testPhone, testAmount2, orderRef2);
