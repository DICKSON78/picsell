...
async function debugChecksum() {
  ...
  console.log(
    "Sending to API:",
    JSON.stringify(payloadWithChecksum, null, 2),
    "\n",
  );
  ...
  try {
    ...
    if (error.response?.data?.message?.includes("checksum")) {
      ...
      console.log(
        "   3. Verify API tokens were regenerated after enabling checksum",
      );
      console.log(
        "   4. Try disabling checksum temporarily to test other parts",
      );
    }
  }
}
