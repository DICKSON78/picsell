...
class AccountScreen extends StatelessWidget {
  ...
  Widget _buildAccountItem(
      IconData icon, String title, Color iconBgColor, VoidCallback onTap) {
    return ListTile(
      leading: Container(
        width: 40,
        height: 40,
      ...
      child: Column(
        children: [
          ...
          Row(
            ...
            children: [
              _buildStat('Credits', '${user?.credits ?? 0}', Icons.bolt,
                  AppTheme.accentOrange),
              _buildStat('Total Spent', 'TZS ${user?.totalSpent.toInt() ?? 0}',
                  Icons.account_balance_wallet, AppTheme.accentGreen),
            ],
          ),
        ],
      ),
  ...
  Widget _buildSettingsSection(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          _buildAccountItem(
              Icons.edit_outlined, 'Edit Profile', AppTheme.primarySoft, () {
            Navigator.pushNamed(context, '/edit-profile');
          }),
          const Divider(height: 1, indent: 64, color: AppTheme.border),
          _buildAccountItem(
              Icons.history_outlined, 'Payment History', AppTheme.iconBgBlue,
              () {
            // Navigator.pushNamed(context, '/payment-history');
          }),
          const Divider(height: 1, indent: 64, color: AppTheme.border),
          _buildAccountItem(Icons.notifications_outlined, 'Notifications',
              AppTheme.iconBgOrange, () {
            Navigator.pushNamed(context, '/notifications');
          }),
          const Divider(height: 1, indent: 64, color: AppTheme.border),
          _buildAccountItem(Icons.account_circle_outlined,
              'Link Google Account', AppTheme.iconBgBlue, () {
            _showLinkGoogleDialog(context);
          }),
          const Divider(height: 1, indent: 64, color: AppTheme.border),
          _buildAccountItem(
              Icons.help_outline, 'Help & Support', AppTheme.iconBgGreen, () {
  ...
  void _showLinkGoogleDialog(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'By linking your Google account, you can sign in faster and have a backup way to access your account.'),
        actions: [
          TextButton(
